<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="./styles/source/normalize.css">
    <link rel="stylesheet" href="./styles/source/clearfix.css">
    <link rel="stylesheet" href="./styles/bantuan.css">
    <title>E-Voting - Home Page</title>
</head>

<body>
    <header>
        <nav>
            <ul class="nav__ul">
                <li class="nav__li"><img class="nav__img" src="./assets/pictures/logo-copas.png" alt="Logo Copas"></li>
                <li class=" nav__li"><a class="nav__a" href="index.php">Beranda</a></li>
                <li class="nav__li"><a class="nav__a" href="#">Quick Count</a></li>
                <li class="nav__li"><a class="nav__a" href="#">Bantuan</a></li>
                <li class="nav__li"><a class="nav__a" href="#">Masuk</a></li>
            </ul>
        </nav>
    </header>
    <main>
        <div id=gambar>
            <section class=banner>
            <div class=judul-bantuan>
                <h1>Bantuan</h1>
            </div>
            <div class=bantuan-img>
                <img src="./assets/pictures/vector-bantuan.svg" alt="Illustrasi Bantuan" width="500">
            </div>
        </div>
        <aside class=deskripsi>
                <pre>
                    <h1>
                        1. Daftarkan diri terlebih dahulu
                        jika belum terdaftar
    
                        2. Pilih "Rekam Jejak" untuk melihat 
                        Rekam Jejak Paslon terkait
    
                        3. Pilih "Coblos" untuk memilih 
                        Paslon yang diinginkan
                    </h1>
                </pre>
            </aside>
        </section>
    </main>
    <footer>
        <h3>
            <p>RPL E-Voting CHOPAS  &copy; 2020</p>
        </h3>
        <h4>
            <p>Ali Naufal Ammarullah, Nurul Akbar Al-Ghifari, Nur Laely Mutmainnah</p>
        </h4>
    </footer>
</body>
</html>
